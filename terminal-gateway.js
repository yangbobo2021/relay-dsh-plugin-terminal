...
const rejected = (code, message) => ({ ok: false, error: { code, message } });
...
export class RelayTerminalGateway extends TypertRemoteService {
  ...
  async list(request) {
    return success([...this.terminals.values()]
      .filter(terminal => terminal.ownerSessionId === request.sessionId)
  ...
  async spawn(request) {
    try {
      ...
      const terminal = {
        terminalId, processId, provider, ownerSessionId: request.sessionId,
        name: request.name, cwd, createdAt: Date.now(), status: { kind: "running" },
        decoder: new StringDecoder("utf8"), text: "", seq: 0, truncated: false,
      };
